...
def main(config: DictConfig) -> None:
    ...
    if "train" in ds.keys() and "validation" not in ds.keys() and "test" not in ds.keys():
        ...
        if config.dataset_args.stratify_by_column is not None:
            unique_sources = sorted(set(train_data[config.dataset_args.stratify_by_column]))
            class_label_feature = ClassLabel(names=unique_sources)
            train_data = train_data.cast_column(config.dataset_args.stratify_by_column, class_label_feature)
