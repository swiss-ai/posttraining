...
def main(args):
    ...
    # train_data = train_data.add_column("prompt_token_ids", tokenizer(train_data["initial_prompt"]["content"][:])["input_ids"])
    ...
    for eval_dataset_name in benchmark_list:
        ...
        with Pool(args.num_proc) as p:  # args.num_proc
        ...
        for train_idx, eval_indices in tqdm(train_idx_match_indices.items()):
            for eval_idx in eval_indices:
                ...
                if match_length / len(eval_ngrams[eval_idx]["tokens"]) >= args.diff_threshold:
                    ...
                    break
        # with Pool(args.num_proc, initializer=init_check_matching, initargs=(train_ngrams, eval_ngrams)) as p:
        #     contamination_mapping = p.starmap(
        #         check_matching,
        #         train_idx_match_indices.items(),
        #     )
        # contamination_mapping = {train_idx: eval_idx for train_idx, eval_idx in contamination_mapping if eval_idx is not None}
    ...
    train_data_output_path = args.train_dataset.replace("03_license_filtered", "04_decontaminated")
    train_data.save_to_disk(train_data_output_path)
