...
"""Offline DPO launcher for a MIXED preference dataset (tokenized + raw HF; combine mode set in config).

The mixture (sources, weights, max_samples, selection, stopping_strategy, total_samples) is encoded in
the dataset config configs/dataset/<dataset_config>.yaml, so this launcher just selects it with
`dataset=<dataset_config>` and keeps the model / DPO hyperparameters. See
src/post_training/data_alignment/tokenized_preference.py (build_mixed_preference_dataset).
"""

stdout_prefix = "init-mixed"
stdout_root = (
    Path(__file__).parent.resolve()
    / f"{stdout_prefix}-{datetime.now().strftime('%Y-%m-%d-%H-%M')}"
)

job_name = "apertus-mixed-dpo"
wandb_project = "Ap1p5-DPO"
wandb_entity = "rkreft-personal"

# Dataset config (configs/dataset/<name>.yaml) that encodes the mixture.
dataset_config = "mllm-dpo-mixed-maxmin"
