...
tasks = ["knowledge", "math", "multilingual", "if", "general", "code", "safety"]
...
run_name = f"capabilities-llama-8b-sweep"
nruns = 0
        ...
        for model in models:
            for lr in learning_rates:
                for plw in prompt_loss_weight:
                    model_config = f"{model}-{dataset}-task-{task}"
                    ...
                    command = (
                        ...
                        f"dataset={dataset} "
                        f"dataset_args.train_split.name={task} "
                        ...
                        f"wandb.tags=[prod,{task},{trainer}] "
